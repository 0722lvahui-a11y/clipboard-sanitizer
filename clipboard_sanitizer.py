...
class App:
    def __init__(self):
        ...
        # 队列: 逐个粘贴的数字
        self.queue = deque()
        self.queue_current = ""  # 当前在剪贴板里的那一项
    ...
    def _build_ui(self):
        ...
        # 状态文字
        self.status = self.canvas.create_text(W // 2, 175, text="🟢  净化中 — 自动去除 act_",
                                               fill=TEXT_DARK, font=FONT_STATUS)
        ...
        # 提示
        self.canvas.create_text(W // 2, 222, text="复制含 act_ 的内容 → Cmd+V 逐个粘贴",
                                 fill=TEXT_HINT, font=FONT_HINT)

        # 底部
        self.canvas.create_rectangle(0, H - 3, W, H, fill=DECO_LINE, outline="")
        self.canvas.create_text(W // 2, H - 12, text="v3.0  队列粘贴  |  macOS",
    ...
    def _toggle(self):
        ...
        if self.enabled:
            ...
            self.canvas.itemconfig(self.status, text="🟢  净化中 — 自动去除 act_")
        else:
    ...
    def _start_key_listener(self):
        ...
        def on_press(key):
            ...
            # 检测 Cmd+V (macOS)
            try:
                is_cmd = (key == Key.cmd or key == Key.cmd_l or key == Key.cmd_r)
            except:
                is_cmd = False
            ...
            if is_cmd:
                self._cmd_held = True
            if is_v and getattr(self, '_cmd_held', False):
                # Cmd+V detected — 先让系统完成粘贴，再切换下一个
